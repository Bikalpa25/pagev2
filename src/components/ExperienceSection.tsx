        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 pb-6 border-b border-slate-100 gap-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1 max-w-xl">
              Chronological track of R&D engineering leadership, startup
              ventures, academic research, and medical fabrication.
            </p>
          </div>
            ...
            <div className="space-y-10 sm:space-y-12">
              {experienceList.map((exp, idx) => {
                const isPresent =
                  ...
                  exp.period.toLowerCase().includes("ongoing");

                return (
                  <div
                    key={exp.id || idx}
                    className="relative group flex flex-col sm:flex-row items-start gap-4 sm:gap-8"
                  >
                    {/* Left Column: Date & Badge (Width-constrained on desktop) */}
                    <div className="sm:w-36 md:w-44 shrink-0 text-left sm:text-right pt-0.5 pr-4">
                    ...
                    <div className="hidden sm:flex absolute left-36 md:left-44 -translate-x-1/2 top-1.5 w-6 h-6 rounded-full bg-white border-2 border-indigo-600 items-center justify-center z-10 group-hover:scale-125 transition-transform shadow-xs">
                      <div
                        className={`w-2 h-2 rounded-full ${isPresent ? "bg-emerald-600" : "bg-indigo-600"}`}
                      ></div>
                    </div>
            ...
            <div className="mt-12 bg-slate-900 text-white rounded-2xl p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border border-slate-800">
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
                  Certified SOLIDWORKS Associate in Mechanical Design. Expert in
                  3D parametric part modeling, complex assembly layout,
                  engineering drawing creation, and FEA stress simulation.
                </p>
              </div>
